# Date: 19.03.2020
# Original passgen author: Bednakov D L
...
class PassGen:
    ...
    def question(self, target):
        ...
        answers['firstname'] = self.prompt('Ввести {}\ Имя: '.format(target))
        answers['lastname'] = self.prompt('Ввести {}\ Фамилию: '.format(target))
        answers['nickname'] = self.prompt('Ввести {}\ Никнейм: '.format(target))

        while True:
            bday = self.prompt('Ввести {}\ день рождения (дд.мм.гггг): '.format(target))
            ...
            for _ in bday.split('.'):
                if not _.isdigit():
                    print('День рождения требует только цифры\n')
                    continue
    ...
    def format_names(self):
        for _ in range(self.max_count_main):
            if not self.silent:
                print(f'Bednakov-Xack-Live: {len(self.passwords)}')
    ...
    def generator(self, ignore_additional = True):
        ...
        self.spouse = self.question('супруги')
        print('\n')

        self.child = self.question('ребенок')
        print('\n')

        self.pet = self.question('домашнее животное')
        print('\n')

        print('Bednakov-Xack-Live... \nIt\'может занять некоторое время.')
        self.format_names()
        if self.silent:
            print("...Генерация {} основные пароли".format(len(self.passwords)))
        ...
        if not ignore_additional:
            print("Генерация дополнительных комбинаций...")
            with open(output_file, 'at') as f:
                i = 0
                while(i < self.max_count):
                    if not self.silent:
                        print('Написание дополнительных комбинаций... {}/{}'.format(i*3, self.max_count*3))
                    f.write('{}{}\n'.format(self.target['firstname'], i))
                    f.write('{}{}\n'.format(self.target['lastname'], i))
                    f.write('{}{}\n'.format(self.target['nickname'], i))
                    i += 1
            if self.silent:
                print("...generated {} дополнительные комбинации".format(self.max_count*3))

        print('Bednakov-Xack-Live Готово , Пароли созданные в файле: {}'.format(output_file))
        quit()
